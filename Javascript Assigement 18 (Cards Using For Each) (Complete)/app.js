...
//   };
  ...
  fruits.forEach(function (data, ind) {
    ...
    <div class="flex relative h-[250px]">
      ...
      <div id = ${ind} class="px-8 py-10  relative z-10 w-full border-4 border-gray-200 bg-white opacity-0 hover:opacity-100">
        ...
        <button class = 'ms-4 px-3 h-[30px]ps-4 outline-primary bg-green-900 text-yellow-300' onclick="updateDesc(this)">Update</button>
        <button class = 'ms-4 px-3 h-[30px]ps-4 outline-primary bg-blue-900 text-cyan-500' onclick="deleteObj(this)">Delete</button>
      </div>
    </div>
  </div>`;
    list.innerHTML += ele;
  ...
  var remove_parent = document.getElementById("remove_parent"); // Extra
  function deleteObj(ele) {
    // console.log("ele=>", ele.previousElementSibling);
    // console.log("ele parentr=>", ele.parentElement);
    // var index = ele.parentElement.id;
    // var removeparent = remove_parent.childNodes; // Extra
    // console.log("removeperent"); // Extra
    // removeperent.remove(); // Extra
    // .remove(); // Extra
    var indexx = ele.parentElement.parentElement; // Extra
    indexx.remove(); // Extra
    console.log("indexx"); // Extra
    ...
    // fruits.forEach(function (data, ind) {
      ...
      <div class="flex relative h-[250px]">
        ...
        <div id = ${ind} class="px-8 py-10  relative z-10 w-full border-4 border-gray-200 bg-white opacity-0 hover:opacity-100">
          ...
          <p class="leading-relaxed">${data.types.join(" , ")}</p>
          <p class="leading-relaxed">${data.desc} </p>
          <button class= 'ms-4 px-3 h-[30px]ps-4 outline-primary' onclick="deleteObj(this)">Delete</button>
        </div>
      </div>
    </div>`;
      list.innerHTML += ele;
